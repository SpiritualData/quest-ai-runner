...
Required env:
  ...
  ANTHROPIC_API_KEY for the reference model provider (planner/answer/models.list)
...
def build_config(*, with_model_provider: bool = True) -> RunnerConfig:
    ...
    ``with_model_provider=False`` omits the AnthropicProvider so a test can inject a stub
    provider without needing an ANTHROPIC_API_KEY.
    """
    ...
    return RunnerConfig(
        ...
        retrieval=retrieval,
        model_provider=AnthropicProvider() if with_model_provider else None,
        deep_runner=deep,
        corpus_root=corpus,
