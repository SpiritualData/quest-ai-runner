...
BUNDLED_DEFAULT_QUEST_INSTRUCTIONS = (
    ...
    "already engaged with. Refine that file in place rather than writing a new one: a fresh "
    "one-off document costs a later run tokens rediscovering which of two files is current, on top "
    "of whatever it cost to write the duplicate. Write a new file only for something genuinely new. "
    "Keep drafts, scratch work, and one-time dated updates in a subfolder of their own rather than "
    "loose beside the standing documents, so what is current stays easy to find.\n\n"
